...
import com.pj.magic.gui.tables.models.BadStockReportItemsTableModel;
...
@Component
public class BadStockReportItemsTable extends MagicTable {
	...
	@Autowired private BadStockReportItemsTableModel tableModel;
	...
	@Autowired
	public BadStockReportItemsTable(BadStockReportItemsTableModel tableModel) {
		super(tableModel);
		initializeColumns();
	...
	public void switchToEditMode() {
		...
		addMode = false;
		List<BadStockReportItem> items = badStockReport.getItems();
	...
	public BadStockReportItemRowItem getCurrentlySelectedRowItem() {
		return tableModel.getRowItem(getSelectedRow());
	}
	
	private boolean hasDuplicate(String unit, BadStockReportItemRowItem rowItem) {
		for (BadStockReportItem item : badStockReport.getItems()) {
	...
	public void setBadStockReport(BadStockReport badStockReport) {
		...
		tableModel.setBadStockReport(badStockReport);
		previousSelectProductCriteria = null;
	}
	
	private BadStockReportItem createBlankItem() {
		BadStockReportItem item = new BadStockReportItem();
		item.setParent(badStockReport);
		return item;
	}
	...
	public void highlightColumn(BadStockReportItem item, int column) {
		int row = badStockReport.getItems().indexOf(item);
	...
	private class UnitCellEditor extends MagicCellEditor {
		...
		public boolean stopCellEditing() {
			...
			if (StringUtils.isEmpty(unit)) {
				...
				BadStockReportItemRowItem rowItem = getCurrentlySelectedRowItem();
