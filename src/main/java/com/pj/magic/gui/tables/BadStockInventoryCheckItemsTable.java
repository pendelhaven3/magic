...
import com.pj.magic.gui.tables.models.BadStockInventoryCheckItemsTableModel;
...
@Component
public class BadStockInventoryCheckItemsTable extends MagicTable {
	...
	public static final int QUANTITY_CHANGE_COLUMN_INDEX = 4;
	...
	@Autowired private BadStockInventoryCheckItemsTableModel tableModel;
	...
	@Autowired
	public BadStockInventoryCheckItemsTable(BadStockInventoryCheckItemsTableModel tableModel) {
		super(tableModel);
		initializeColumns();
	...
	private void initializeColumns() {
		...
		columnModel.getColumn(QUANTITY_CHANGE_COLUMN_INDEX).setPreferredWidth(50);
	...
	public void switchToEditMode() {
		...
		addMode = false;
		List<BadStockInventoryCheckItem> items = badStockInventoryCheck.getItems();
	...
	public BadStockInventoryCheckItemRowItem getCurrentlySelectedRowItem() {
		return tableModel.getRowItem(getSelectedRow());
	}
	
	private boolean hasDuplicate(String unit, BadStockInventoryCheckItemRowItem rowItem) {
		for (BadStockInventoryCheckItem item : badStockInventoryCheck.getItems()) {
	...
	public void setBadStockInventoryCheck(BadStockInventoryCheck badStockInventoryCheck) {
		...
		tableModel.setBadStockInventoryCheck(badStockInventoryCheck);
		previousSelectProductCriteria = null;
	}
	
	private BadStockInventoryCheckItem createBlankItem() {
	...
	public void highlightColumn(BadStockInventoryCheckItem item, int column) {
		int row = badStockInventoryCheck.getItems().indexOf(item);
	...
	private class UnitCellEditor extends MagicCellEditor {
		...
		public boolean stopCellEditing() {
			...
			if (StringUtils.isEmpty(unit)) {
				...
				BadStockInventoryCheckItemRowItem rowItem = getCurrentlySelectedRowItem();
