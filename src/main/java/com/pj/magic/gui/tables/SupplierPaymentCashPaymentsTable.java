...
import com.pj.magic.gui.tables.models.SupplierPaymentCashPaymentsTableModel;
...
@Component
public class SupplierPaymentCashPaymentsTable extends MagicTable {
	...
	@Autowired private SupplierPaymentCashPaymentsTableModel tableModel;
	...
	@Autowired
	public SupplierPaymentCashPaymentsTable(SupplierPaymentCashPaymentsTableModel tableModel) {
		super(tableModel);
		initializeColumns();
	...
	public void addNewRow() {
		...
		cashPayment.setParent(supplierPayment);
	...
	public void doDeleteCurrentlySelectedItem() {
		...
		SupplierPaymentCashPayment item = getCurrentlySelectedRowItem().getCashPayment();
		clearSelection(); // clear row selection so model listeners will not cause exceptions while model items are being updated
		supplierPayment.getCashPayments().remove(item);
	...
	protected void registerKeyBindings() {
		...
		*/
		...
		actionMap.put(F5_ACTION_NAME, new AbstractAction() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (isPaidDateColumnSelected() && supplierPayment.isNew()) {
	...
	public SupplierPaymentCashPaymentRowItem getCurrentlySelectedRowItem() {
