...
import com.pj.magic.gui.tables.models.SupplierPaymentBankTransfersTableModel;
...
@Component
public class SupplierPaymentBankTransfersTable extends MagicTable {
	...
	@Autowired private SupplierPaymentBankTransfersTableModel tableModel;
	@Autowired private SelectDateDialog selectDateDialog;
	
	private SupplierPayment supplierPayment;
	
	@Autowired
	public SupplierPaymentBankTransfersTable(SupplierPaymentBankTransfersTableModel tableModel) {
		super(tableModel);
		initializeColumns();
	...
	public void addNewRow() {
		...
		bankTransfer.setParent(supplierPayment);
	...
	public void doDeleteCurrentlySelectedItem() {
		...
		SupplierPaymentBankTransfer item = getCurrentlySelectedRowItem().getBankTransfer();
		clearSelection(); // clear row selection so model listeners will not cause exceptions while model items are being updated
		supplierPayment.getBankTransfers().remove(item);
	...
	protected void registerKeyBindings() {
		...
		*/
		...
		actionMap.put(F5_ACTION_NAME, new AbstractAction() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (isTransferDateColumnSelected() && supplierPayment.isNew()) {
	...
	public SupplierPaymentBankTransferRowItem getCurrentlySelectedRowItem() {
