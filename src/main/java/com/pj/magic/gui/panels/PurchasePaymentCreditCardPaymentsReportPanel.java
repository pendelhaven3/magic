...
public class PurchasePaymentCreditCardPaymentsReportPanel extends StandardMagicPanel {
	...
	private static final int CREDIT_CARD_COLUMN_INDEX = 2;
	private static final int AMOUNT_COLUMN_INDEX = 3;
	private static final int TRANSACTION_DATE_COLUMN_INDEX = 4;
	private static final int APPROVAL_CODE_COLUMN_INDEX = 5;
	...
	private JLabel totalCreditCardPaymentsField = new JLabel();
	...
	private void generateReport() {
		...
		PurchasePaymentCreditCardPaymentsReport report = doGenerateReport();
		tableModel.setCreditCardPayments(report.getCreditCardPayments());
		if (!report.getCreditCardPayments().isEmpty()) {
			table.changeSelection(0, 0);
		}
		totalCreditCardPaymentsField.setText(FormatterUtil.formatAmount(report.getTotalAmount()));
	...
	private PurchasePaymentCreditCardPaymentsReport doGenerateReport() {
		PurchasePaymentCreditCardPaymentsReport report = new PurchasePaymentCreditCardPaymentsReport();
	...
	private JPanel createTotalsPanel() {
		...
		c.gridy = currentRow;
		c.anchor = GridBagConstraints.WEST;
		totalCreditCardPaymentsField = ComponentUtil.createRightLabel(120, "");
		mainPanel.add(totalCreditCardPaymentsField, c);
...
//		printPreviewDialog.setVisible(true);
	...
	private class CreditCardPaymentsTableModel extends AbstractTableModel {
		...
		private final String[] columnNames = 
			{"PP No.", "Supplier", "Credit Card", "Amount", "Transfer Date", "Reference Number"};
		
		private List<PurchasePaymentCreditCardPayment> creditCardPayments = new ArrayList<>();
		
		public void setCreditCardPayments(List<PurchasePaymentCreditCardPayment> bankTransfers) {
			this.creditCardPayments = bankTransfers;
