...
@Component
public class MaintainAdjustmentTypePanel extends StandardMagicPanel {

	private static final Logger logger = LoggerFactory.getLogger(MaintainAdjustmentTypePanel.class);
	...
	protected void saveAdjustmentType() {
		...
		if (confirm("Save?")) {
			...
			try {
				adjustmentTypeService.save(adjustmentType);
				showMessage("Saved!");
	...
	private boolean validateAdjustmentType() {
		...
		AdjustmentType existing = adjustmentTypeService.findAdjustmentTypeByCode(codeField.getText());
		if (existing != null) {
			if (adjustmentType.getId() == null || !existing.getId().equals(adjustmentType.getId())) {			
				showErrorMessage("Code is already used by another Adjustment Type record");
	...
	public void updateDisplay(AdjustmentType type) {
