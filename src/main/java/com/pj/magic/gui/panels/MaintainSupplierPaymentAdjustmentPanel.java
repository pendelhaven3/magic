...
@Component
public class MaintainSupplierPaymentAdjustmentPanel extends StandardMagicPanel {

	private static final Logger logger = LoggerFactory.getLogger(MaintainSupplierPaymentAdjustmentPanel.class);
	...
	private void savePaymentAdjustment() {
		...
		if (confirm("Save?")) {
			...
			try {
				supplierPaymentAdjustmentService.save(paymentAdjustment);
				showMessage("Saved!");
				getMagicFrame().switchToEditSupplierPaymentAdjustmentPanel(paymentAdjustment);
			} catch (Exception e) {
	...
	public void updateDisplay(SupplierPaymentAdjustment paymentAdjustment) {
		...
		this.paymentAdjustment = paymentAdjustment = 
				supplierPaymentAdjustmentService.getSupplierPaymentAdjustment(paymentAdjustment.getId());
		
		boolean posted = paymentAdjustment.isPosted();
		
		paymentAdjustmentNumberLabel.setText(String.valueOf(paymentAdjustment.getSupplierPaymentAdjustmentNumber()));
...
//		toolBar.add(postButton);
	...
	private void postPaymentAdjustment() {
		if (confirm("Do you want to post this Supplier Payment Adjustment?")) {
			try {
				supplierPaymentAdjustmentService.post(paymentAdjustment);
				showMessage("Supplier Payment Adjustment posted");
