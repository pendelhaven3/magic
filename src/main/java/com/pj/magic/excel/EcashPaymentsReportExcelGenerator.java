...
public class EcashPaymentsReportExcelGenerator {

    public Workbook generate(EcashPaymentsReport report) {
        ...
        cell.setCellValue("E-CASH PAYMENTS REPORT");
        ...
        for (PaymentEcashPayment payment : report.getPayments()) {
            currentRow++;
            row = sheet.createRow(currentRow);
            ...
            cell = row.createCell(3);
            cell.setCellValue(FormatterUtil.formatDate(payment.getReceivedDate()));
            cell.setCellStyle(dateStyle);
            
            row.createCell(4).setCellValue(payment.getReceivedBy().getUsername());
            row.createCell(5).setCellValue(String.valueOf(payment.getParent().getPaymentNumber()));
        }
