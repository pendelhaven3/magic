...
@Getter
@Setter
public class CellphonePromoRaffleTicketClaimSummary {
	...
	public int getNumberOfTickets() {
		return getTotalAmount().divideToIntegralValue(PromoServiceImpl.JCHS_CELLPHONE_RAFFLE_SALES_AMOUNT_PER_TICKET).intValue();
	}
	...
	public static List<CellphonePromoRaffleTicketClaimSummary> toSummaries(List<SalesInvoice> salesInvoices) {
		...
		List<CellphonePromoRaffleTicketClaimSummary> summaries = new ArrayList<>();
		
		CellphonePromoRaffleTicketClaimSummary summary = null;
		for (SalesInvoice salesInvoice : salesInvoices) {
			if (summary == null || !summary.getTransactionDate().equals(salesInvoice.getTransactionDate())) {
				summary = new CellphonePromoRaffleTicketClaimSummary();
